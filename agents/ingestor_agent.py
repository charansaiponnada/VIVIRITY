...
SECTION_QUERIES = {
    "identity":      ["company name","cin","directors","promoter","incorporated","board of directors","corporate governance","chairman","managing director","shareholding pattern"],
...
PRIORITY_SECTIONS = ["ebitda","balance_sheet","current_items","net_worth","debt","ratios","notes"]
...
BANKING_PRIORITY_SECTIONS = ["npa","capital","deposits","advances","balance_sheet","ratios","income"]


class IngestorAgent:
    def __init__(self, file_paths: list[str], log_callback=None, entity_type: str = "corporate",
                 extract_schema: dict = None, custom_fields: str = ""):
        self.file_paths = file_paths
        self.log        = log_callback or print
        self.model      = "gemini-2.5-flash"
        self.client     = genai.Client(api_key=os.getenv("GEMINI_API_KEY"))
        self._entity_type = entity_type
        self.extract_schema = extract_schema or {"ratios": True, "directors": True, "red_flags": True}
        self.custom_fields = [f.strip() for f in (custom_fields or "").split(",") if f.strip()]
        ...
        for path in self.file_paths:
            ...
            try:
                ...
                if ext == ".json":
                    ...
                    results[doc_type] = self._merge_document_data(results.get(doc_type, {}), data, doc_type)
                    continue
                elif ext == ".csv":
                ...
                with pdfplumber.open(path) as pdf:
                    ...
                    self.log(f"→ Classified as: {doc_type.upper().replace('_',' ')} ({page_count} pages)")
                    ...
                    results[doc_type] = self._merge_document_data(results.get(doc_type, {}), extracted, doc_type)
                    if "merged_all" not in results: results["merged_all"] = {}
                    results["merged_all"] = self._merge_document_data(results["merged_all"], extracted, "flat_merge")
    ...
    def _merge_document_data(self, base: dict, incoming: dict, doc_type: str) -> dict:
        if not isinstance(base, dict): return incoming or {}
        if not isinstance(incoming, dict): return base
        merged = dict(base)
        for k, v in incoming.items():
            if k == "red_flags":
                ...
                for item in v:
                    if str(item).lower().strip() not in seen: existing.append(item)
                merged[k] = existing
    ...
    def _extract(self, pdf, path: str, doc_type: str, page_count: int) -> dict:
        if doc_type == "annual_report": return self._extract_annual_report(pdf, path, page_count)
        elif doc_type == "gst_filing": return self._extract_gst(pdf)
        elif doc_type == "itr_filing": return self._extract_itr(pdf)
        elif doc_type == "bank_statement": return self._extract_bank_statement(pdf)
        elif doc_type == "alm_report": return self._extract_alm_report(pdf)
        elif doc_type == "shareholding_pattern": return self._extract_shareholding_pattern(pdf)
        elif doc_type == "borrowing_profile": return self._extract_borrowing_profile(pdf)
        elif doc_type == "portfolio_cuts": return self._extract_portfolio_cuts(pdf)
        else: return self._extract_generic(pdf, doc_type)
    ...
    def _extract_annual_report(self, pdf, path: str, page_count: int) -> dict:
        ...
        for pnum in range(len(doc)):
            ...
            if any(any(kw in txt for kw in sq[s]) for s in prio): selected_pages.add(pnum)
            if len(selected_pages) >= PAGE_BUDGET: break
        doc.close()
    ...
    def _ai_extract_unified_financials(self, text: str, tables_str: str, entity_type: str) -> dict:
        ...
        directors_p = '    "directors": [], "promoters": [],' if self.extract_schema.get("directors", True) else ""
        red_flags_p = '    "red_flags": {"audit_qualified": false, "going_concern_issue": false},' if self.extract_schema.get("red_flags", True) else ""
        custom_p = f'\n    # CUSTOM: {", ".join(self.custom_fields)}' if self.custom_fields else ""
...
"""
        ...
        except Exception: return {"red_flags": {}}
    ...
    def _compute_ratios(self, data: dict) -> dict:
        if not data or data.get("parse_error"): return data
        notes = str(data.get("extraction_notes", "")).lower()
        if data.get("total_borrowings_crores") is None:
            if any(m in notes for m in ["debt free", "zero debt", "nil debt"]): data["total_borrowings_crores"] = 0.0
        
        def sf(v):
            try: return float(str(v).replace(",","")) if v is not None else None
            except: return None
        
        rev, ebitda, debt, nw = sf(data.get("revenue_crores")), sf(data.get("ebitda_crores")), sf(data.get("total_borrowings_crores")), sf(data.get("net_worth_crores"))
        if nw is None and data.get("total_assets_crores") and data.get("total_liabilities_crores"):
            nw = sf(data["total_assets_crores"]) - sf(data["total_liabilities_crores"])
            data["net_worth_crores"] = round(nw, 2)
        
        if debt is not None and nw and nw > 0: data["debt_equity_ratio"] = round(debt/nw, 2)
        if rev and rev > 0 and ebitda is not None: data["ebitda_margin_percent"] = round(ebitda/rev*100, 1)
        return data
    ...
    def _extract_pages(self, pdf, page_nums: list[int]) -> tuple[str, str]:
        ...
        for pn in sorted(set(page_nums)):
            ...
            if txt.strip(): texts.append(f"[P{pn+1}] {txt}")
            for t in (p.extract_tables() or []):
                if t and len(t) > 1: tables.append(t[:15])
    ...
    def _extract_alm_report(self, pdf) -> dict:
        ...
        return self._ai_call(f"Extract ALM data (maturity buckets, gap). Text: {txt[:8000]} Tables: {tbls[:4000]}")

    def _extract_shareholding_pattern(self, pdf) -> dict:
        txt, tbls = self._extract_pages(pdf, list(range(min(10, len(pdf.pages)))))
        return self._ai_call(f"Extract Shareholding (Promoter %, Public %). Text: {txt[:8000]} Tables: {tbls[:4000]}")

    def _extract_borrowing_profile(self, pdf) -> dict:
        txt, tbls = self._extract_pages(pdf, list(range(min(15, len(pdf.pages)))))
        return self._ai_call(f"Extract Borrowings (Lenders, Limits). Text: {txt[:8000]} Tables: {tbls[:4000]}")

    def _extract_portfolio_cuts(self, pdf) -> dict:
        txt, tbls = self._extract_pages(pdf, list(range(min(20, len(pdf.pages)))))
        return self._ai_call(f"Extract Portfolio Performance (NPA buckets). Text: {txt[:8000]} Tables: {tbls[:4000]}")

    def _extract_generic(self, pdf, doc_type: str) -> dict:
        txt, _ = self._extract_pages(pdf, [0,1,2])
        return self._compute_ratios(self._ai_call(f"Extract financials from {doc_type}. Text: {txt[:8000]}"))
    ...
    def _ingest_csv(self, path: str) -> dict:
        with open(path, "r") as f: reader = csv.DictReader(f); rows = list(reader)
        return {"_doc_type": "bank_statement", "revenue_crores": len(rows), "transaction_count": len(rows)}
